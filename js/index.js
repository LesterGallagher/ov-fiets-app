...
document.addEventListener('init', function(event) {
  ...
  if (page.id === 'main-page') {
    page.querySelector('#go-button').onclick = function() {
        ...
        document.querySelector('#Navigator').pushPage('fietsen.html', {data: {title: selected_station + " OV Fietsen"}});
    };
	...
	var abrev = window.station_abrevs[window.selected_station];
	window.currstation = window.station_data[abrev];
...
function RequestLocalData(){
	httpGetJSONRequest("data/station_abrevs.json",function(data){
		//Critical!!
		//console.log(data);
		station_abrevs = data;
	},"Were sorry, an error occurred.");//Error message when something goes wrong
}
function RequestAPIData(callback = undefined){
...
function onStationInput(event,length = 8){
	...
	for(let key in station_abrevs)
	{
		if (regex.test(key)){//If input 
			var currstation = window.station_data[station_abrevs[key]];
...
{
	var sortedStations = Object.keys(window.station_data);
	sortedStations.sort(function(a, b)
	{
		return (window.station_data[b].open.toLocaleLowerCase() === "yes" ? window.station_data[b].extra.rentalBikes : 0) 
		- (window.station_data[a].open.toLocaleLowerCase() === "yes" ? window.station_data[a].extra.rentalBikes : 0);
	});
	...
	for(let i = 0; i < length; i ++)
	{
		...
		optionHref.setAttribute("class","left");
		optionListItem.onclick = function(event){
			inputElement.value = window.station_data[sortedStations[i]].description;
			selected_station = window.station_data[sortedStations[i]].description;
		};
		optionHref.innerHTML = "<p>" + window.station_data[sortedStations[i]].description + "</p>";
		optionListItem.appendChild(optionHref);
		...
		var isOpen = (window.station_data[sortedStations[i]].open.toLowerCase() == "yes");

		
		rightelem.innerHTML = isOpen ? "<ons-icon icon='ion-android-bicycle'>&#32;"+(window.station_data[sortedStations[i]].extra.rentalBikes || 0)+"</ons-icon>"
		: "<ons-icon icon='ion-alert-circled' color='red'></ons-icon>";
...
function setupSecondaryPage(){
	var abrev = window.station_abrevs[window.selected_station];
	var currstation = window.station_data[abrev];
